...
const dcaGuardian: ModelDef = {
  ...
  hedge: "レバレッジ1x・ショートなし・損切りなし（時間分散でリスクを平準化）。",
  expectedReturn: "低・安定（原資産に連動しつつ変動を抑制）",
  risk: "low",
  kind: "dca",
  dcaEveryN: 7,
  params: { leverage: 1, positionPct: 0.1, stopLossPct: null, takeProfitPct: null, allowShort: false },
  prepare: () => ({}),
};
...
const rangeSniper: ModelDef = {
  ...
  hedge: "レバ2x・ロングのみ・損切り-5%/利確+8%でレンジ逸脱を限定。",
  expectedReturn: "中低（勝率重視・1回の利幅は小さめ）",
  risk: "mid_low",
  kind: "single",
  params: { leverage: 2, positionPct: 0.2, stopLossPct: 0.05, takeProfitPct: 0.08, allowShort: false },
...
const trendRider: ModelDef = {
  ...
  hedge: "レバ4x・損切り-6%・利確なし（トレンド継続を取りきる）。ドテンで方向転換に追随。",
  expectedReturn: "中（トレンド相場で伸び、レンジでは往復ビンタに弱い）",
  risk: "mid",
  kind: "single",
  params: { leverage: 4, positionPct: 0.25, stopLossPct: 0.06, takeProfitPct: null, allowShort: true },
...
const breakoutHunter: ModelDef = {
  ...
  hedge: "レバ8x・損切り-4%・利確+12%。ダマシに備えタイトな損切りで即撤退。",
  expectedReturn: "中高（当たれば大きいがダマシで細かい損失も出やすい）",
  risk: "mid_high",
  kind: "single",
  params: { leverage: 8, positionPct: 0.2, stopLossPct: 0.04, takeProfitPct: 0.12, allowShort: true },
  prepare: (c) => ({
...
const momentumBlitz: ModelDef = {
  ...
  hedge: "レバ15x（ハイリスク）だが損切り-2%/利確+3%を厳守し1回の損失を極小化。",
  expectedReturn: "高・高変動（資産曲線の振れが大きい。ドローダウンも大）",
  risk: "high",
  kind: "single",
  params: { leverage: 15, positionPct: 0.15, stopLossPct: 0.02, takeProfitPct: 0.03, allowShort: true },
